...
set GOOS=linux
go build -o main common.go send_live_chat_message.go create_new_room.go
C:\Users\momom\go\bin\build-lambda-zip.exe -output main.zip main
aws lambda create-function --function-name create_new_room --runtime go1.x --zip-file fileb://main.zip --handler main --role arn:aws:iam::652333062396:role/service-role/my-first-golang-lambda-function-role-cb8uw4th
aws lambda update-function-code --function-name create_new_room --zip-file fileb://main.zip
